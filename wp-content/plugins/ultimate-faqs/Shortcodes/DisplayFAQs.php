...
function Display_FAQs($atts) {
	...
	foreach ($Category_Array as $Category) {
	    ...
	    while ( $FAQ_Query->have_posts() ): $FAQ_Query->the_post(); global $post;
				...
				else {$ReturnString .= "'></span><i class='search-icon fa fa-search' id='toggle-how-to-use-qa'></i></div>";}

				$ReturnString .= "<div class='ufaq-faq-title-text'><" . $UFAQ_Styling_FAQ_Heading_Type . " itemprop='name'>" . apply_filters('the_title', $faq->post_title, $faq->ID) . "</" . $UFAQ_Styling_FAQ_Heading_Type . "></div><span class='right-arrow-toggle-qa-icon'><img src='/wp-content/uploads/2018/05/btn_expand_more.png' id='qa-item-right-arrow-toggle'></img></span><div class='ewd-ufaq-clear'></div></a>";
				$ReturnString .= "</div>";
				...
				foreach ($FAQ_Elements as $FAQ_Element) {
					...
					if ($FAQ_Element == "Body") {
						$ReturnString .= "<div class='ewd-ufaq-post-margin ufaq-faq-post' id='ufaq-post-" . $faq->ID . "' itemprop='text'> <pre id='faq-body-content-inner-wrap'>" . strip_tags ( $faq->post_content, '<span>') . "</pre></div>";
					}
					...
					if ($FAQ_Element == "Custom_Fields" and sizeOf($FAQ_Fields_Array) > 0) {
						...
						foreach ($FAQ_Fields_Array  as $FAQ_Field_Item) {
							...
							if ($Hide_Blank_Fields != "Yes" or $Value != "") {
								...
								elseif ($FAQ_Field_Item['FieldType'] == "link") {$ReturnString .= "< href='" . $Value . "' target='_blank'>" . $Value ."<span>";}
