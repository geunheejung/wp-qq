jQuery(function(){ //DOM Ready
 ...
 if (jQuery(window).width() >= 767) {
    jQuery('#1522207757968-2-4').attr('style', 'border-left: 0px;');
  }
  ...
  jQuery(window).resize(function () {
    ...
    if (jQuery(window).width() <= 767) {
      ...
      jQuery('#1522207757968-2-4').attr('style', 'border-left: 1px solid #ccc;');
...
function EWD_UFAQ_Reveal_FAQ(post_id, selectedIDString) {
  ...
  var currentItem = jQuery('#ewd-ufaq-post-symbol-'+post_id);
  var leftQaItemSearchToggle = currentItem.next('#toggle-how-to-use-qa');
  var rightQaItemArrowToggle = currentItem.parent('.ewd-ufaq-post-margin-symbol').siblings('.right-arrow-toggle-qa-icon').children('img');
  rightQaItemArrowToggle.attr('src', '/wp-content/uploads/2018/05/btn_expand_less.png')
  leftQaItemSearchToggle.addClass('actvie');
  ...
  if (reveal_effect != "none") {runEffect("show", post_id); }
    ...
    jQuery('#ufaq-body-'+post_id).removeClass("ewd-ufaq-hidden");
    jQuery('#ufaq-body-'+post_id).stop().slideDown(300);
  }
...
function EWD_UFAQ_Hide_FAQ(post_id) {
  ...
  var leftQaItemSearchToggle = currentItem.next('#toggle-how-to-use-qa');
  var rightQaItemArrowToggle = currentItem.parent('.ewd-ufaq-post-margin-symbol').siblings('.right-arrow-toggle-qa-icon').children('img');
  rightQaItemArrowToggle.attr('src', '/wp-content/uploads/2018/05/btn_expand_more.png')
  leftQaItemSearchToggle.removeClass('actvie');

  if (reveal_effect != "none") {runEffect("hide", post_id);}
  else {
    jQuery('#ufaq-body-'+post_id).addClass("ewd-ufaq-hidden");
    jQuery('#ufaq-body-'+post_id).stop().slideUp(300);
  }
